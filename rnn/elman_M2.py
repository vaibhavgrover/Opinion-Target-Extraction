...
class model(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, em, init, featdim):
        ...
        # bundle weights
        self.params = [self.emb, self.W_xz, self.W_hz, self.b_z, self.W_hh, self.W_xh, \
                        self.b_h, self.W_hr,  self.b_r, self.W_hy, self.b_y, self.h0]
        self.names  = ['embeddings', 'W_xz', 'W_hz', 'b_z', 'W_hh', 'W_xh',  'b_h', \
                       'W_hr', 'b_r', 'W_hy', 'b_y', 'h0']
        ...
        def recurrence(x_t, feat_t, h_tm1):
            ...
            ###### THIS IS DIFFERENT
            r_t = sigma(x_t[50:100] + theano.dot(h_tm1, self.W_hr) + self.b_r)
            h_t = (T.tanh( theano.dot(h_tm1*r_t, self.W_hh) +theano.dot(x_t, self.W_xh)+ self.b_h)*z_t) + h_tm1*(T.ones_like(z_t) - z_t)
            # h_t = T.tanh(h_tm1)
