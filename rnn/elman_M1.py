...
class model(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, em, init, featdim):
        ...
        # bundle weights
        self.params = [self.emb, self.W_xz, self.b_z, self.W_xr,self.W_hr,  self.b_r, \
                        self.W_hh, self.b_h,  self.W_hy, self.b_y, self.h0]
        self.names  = ['embeddings', 'W_xz', 'b_z', 'W_xr', 'W_hr', 'b_r', \
                        'W_hh', 'b_h', 'W_hy', 'b_y', 'h0']
        ...
        def recurrence(x_t, feat_t, h_tm1):
            ...
            h_t = (T.tanh( theano.dot(h_tm1*r_t, self.W_hh) +T.tanh(x_t[50:100])+ self.b_h)*z_t) + h_tm1*(T.ones_like(z_t) - z_t)
            # h_t = T.tanh(h_tm1)
